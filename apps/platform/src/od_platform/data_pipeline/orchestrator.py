...
# @Author    :雨霓同学
# @Project   :ODPlatform
# @Function  :数据流水线编排器 —— 将 转换→报告→划分→落盘→yaml 串成一条龙
...
from od_platform.data_pipeline.split.manifest import Pair, PairList
...
class DatasetPipeline:
    ...
    def run(self) -> Dict:
        ...
        # task 校验由 convert/service.py 在分发时完成，此处不重复。
        # 转换的中间产物写进系统临时目录,出了 with 自动清理 —— 全程不碰 data/raw/。
    ...
    def _pair_images_with_labels(self, labels_dir: Path) -> PairList:
        ...
        图像索引遍历 IMAGE_EXTENSIONS,所以 png/jpeg/... 都不会被漏。
